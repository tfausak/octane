...
import Octane.Types.Int32LE (Int32LE (NewInt32LE), unInt32LE)
...
newtype ActorMap = NewActorMap
    { unActorMap :: M.IntMap PCString
    } deriving (Show)

instance B.Binary ActorMap where
    get = do
        NewList list <- B.get
        return (NewActorMap (M.fromList (map (\ a -> (unInt32LE (actorTag a), actorName a)) list)))
